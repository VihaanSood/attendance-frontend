  ...
  return (
    <div className="animate-up">
      ...
      <div className="grid-4 section">
        <StatCard label="Total Payroll"   value={report ? `$${report.totalPayroll?.toLocaleString()}` : '—'} icon={DollarSign}   color="accent" loading={loading} />
      ...
      <Table
        ...
        columns={[
          ...
          { key: 'monthlySalary', label: 'Monthly Salary',
            render: v => <span style={{ fontFamily: 'var(--font-mono)', color: 'var(--text-secondary)' }}>${Number(v).toLocaleString()}</span> },
          ...
          { key: 'salaryPerDay', label: 'Per Day',
            render: v => <span style={{ fontFamily: 'var(--font-mono)', fontSize: 12, color: 'var(--text-muted)' }}>${Number(v).toFixed(2)}</span> },
          { key: 'deductionAmount', label: 'Deduction',
            render: v => <span style={{ fontFamily: 'var(--font-mono)', color: 'var(--red)' }}>-${Number(v).toFixed(2)}</span> },
        ...
        {loadingDetail ? (
          ...
          <div>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 20 }}>
              {[
                { label: 'Monthly Salary',    value: `$${Number(detailData.monthlySalary).toLocaleString()}`,  color: 'var(--text-primary)' },
                { label: 'Salary / Day',      value: `$${detailData.salaryPerDay}`,                            color: 'var(--accent)' },
                ...
                { label: 'Deduction',         value: `-$${detailData.deductionAmount}`,                        color: 'var(--red)' },
              ].map(item => (
            ...
            <div style={{ padding: '16px 20px', background: 'var(--accent-dim)', borderRadius: 10, border: '1px solid var(--accent-glow)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              ...
              <span style={{ fontFamily: 'var(--font-mono)', fontWeight: 900, fontSize: 24, color: 'var(--accent)' }}>
                ${Number(detailData.finalSalary).toLocaleString()}
              </span>
            </div>
          </div>
        ) : null}
