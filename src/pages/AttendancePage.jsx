...
export default function AttendancePage() {
  ...
  // Daily attendance
  const { data: dailyData, loading: dailyLoading, refresh: refreshDaily } = useApi(
  ...
  useEffect(() => {
    const records = dailyData?.data?.data?.records || dailyData?.data?.records
    if (records) {
      const map = {}
      records.forEach(r => { map[r.employeeId] = r.attendance?.status || '' })
      setBulkMap(map)
    }
  }, [dailyData])
