...
export default function LeavesPage() {
  ...
  const { data: leavesData, loading, refresh } = useApi(
    () => leavesApi.list({ status: statusFilter, type: typeFilter, page, limit: 15 }),
    [statusFilter, typeFilter, page]
  )

  const leaves = leavesData?.data?.data || leavesData?.data || []
  const pagination = leavesData?.data?.pagination || leavesData?.pagination
